...
namespace MemoryBoxGit.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> VerifyCode(VerifyCodeViewModel model)
        {
            ...
            {
                case SignInStatus.Success:
                    return RedirectToLocal(model.ReturnUrl, User.Identity.Name);
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.Failure:
                default:
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Username, Email = model.Username + "@x.com" };
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            {
                case SignInStatus.Success:
                    return RedirectToLocal(returnUrl, User.Identity.Name);
                case SignInStatus.LockedOut:
                    return View("Lockout");
                case SignInStatus.RequiresVerification:
        ...
        private ActionResult RedirectToLocal(string returnUrl, string username)
        {
            ...
            //DW 22/11/2016
            //need to perform secondary checks on the users table before redirecting to the ChildStart page
            UserRepository usr = new UserRepository();
            VirtualMemoryBox.Domain.User myUser = new VirtualMemoryBox.Domain.User();
            myUser = usr.GetUser(username);

            if (myUser.AccountStatus == "Active")
            {
                return RedirectToAction("ChildStart", "Home");
            }
            else if (myUser.AccountStatus == "Pending")
            {
                return RedirectToAction("Pending", "Home");
            }
            else if (myUser.AccountStatus == "Locked")
            {
                return RedirectToAction("Pending", "Home");
            }
            else
